...
-- populate DROP NONCLUSTERED INDEX data
...
-- SELECT 
    CASE WHEN A.[is_unique_constraint] = 1 THEN
...
WHERE A.type = 2 -- nonclustered only
AND B.Type != 'S'
...
ORDER BY S.name, B.name, A.index_id DESC;
-- SELECT * FROM [dbo].[CommandExec] WHERE [ObjectType] = 'IX' AND [CommandType] = 'D' ORDER BY [ID] ASC

-- populate CREATE NONCLUSTERED INDEX data
INSERT INTO [dbo].[CommandExec] (
    [DatabaseName], [SchemaName], [ObjectName], [ObjectType], [CommandType], [Command])
SELECT 
...
-- SELECT 
    CAST(CASE 
            WHEN A.type = 1 AND is_unique = 1 THEN 'CREATE UNIQUE CLUSTERED INDEX ' + QUOTENAME(A.[Name]) + ' ON ' + QUOTENAME(S.name) + '.' + QUOTENAME(OBJECT_NAME(A.[object_id]))
            WHEN A.type = 1 AND is_unique = 0 THEN 'CREATE CLUSTERED INDEX ' + QUOTENAME(A.[Name]) + ' ON ' + QUOTENAME(S.name) + '.' + QUOTENAME(OBJECT_NAME(A.[object_id]))
            WHEN A.type = 2 AND is_unique = 1 AND is_unique_constraint = 0 THEN 'CREATE UNIQUE NONCLUSTERED INDEX ' + QUOTENAME(A.[Name]) + ' ON ' + QUOTENAME(S.name) + '.' + QUOTENAME(OBJECT_NAME(A.[object_id]))
            WHEN A.type = 2 AND is_unique = 0 AND is_unique_constraint = 0 THEN 'CREATE NONCLUSTERED INDEX ' + QUOTENAME(A.[Name]) + ' ON ' + QUOTENAME(S.name) + '.' + QUOTENAME(OBJECT_NAME(A.[object_id]))
            WHEN A.type = 2 AND is_unique = 1 AND is_unique_constraint = 1 THEN 'ALTER TABLE ' + QUOTENAME(S.name) + '.' + QUOTENAME(OBJECT_NAME(A.[object_id])) + ' ADD CONSTRAINT ' + + QUOTENAME(A.[Name]) + ' UNIQUE NONCLUSTERED '
          END + ' (' 
        + STUFF((
        ...
        --+ CASE is_unique_constraint WHEN 0 THEN ', DROP_EXISTING = OFF' ELSE '' END
...
AND S.name != 'sys'
AND B.name != 'CommandExec'
ORDER BY S.name, B.name, A.index_id ASC;
-- SELECT * FROM [dbo].[CommandExec] WHERE [ObjectType] = 'IX' AND [CommandType] = 'C' ORDER BY [ID] ASC
